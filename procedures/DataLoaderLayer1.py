...
class data_loader:
    ...
    def load_train(self, augmentation=0):
        ...
        for img in os.listdir(train_path):
            ...
            img_array = self.pad_img(img_array)
            map_array = self.pad_img(map_array)
            r, f = self.break_to_tiles(img_array, map_array,5)
            self.real.extend(r)
            self.fake.extend(f)
        np.random.shuffle(self.real)
        self.real = self.real[0:len(self.fake)*2]
        ...
        self.real_eval = np.copy(self.real[np.arange(0,10*self.val_size,10), :, :, :])
        self.fake_eval = np.copy(self.fake[np.arange(0,10*self.val_size,10), :, :, :])
        print("training set:")
    ...
    def pad_img(self, img):
        new_img = np.zeros((3072, 3072))
        rows, cols = img.shape
        new_img[1536 - rows//2 : 1536  -rows//2 + rows, 1536 - cols//2 : 1536 - cols//2 + cols] = img
        return new_img

#loader = data_loader("D:\\Breast Cancer\\Databases\\Forensic-Transfer\\layer1")
#loader.load_train()
#loader.load_ephoc()
#loader.show_fake()